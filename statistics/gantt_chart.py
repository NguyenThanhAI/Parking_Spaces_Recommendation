...
print(df)
num_hours = range(int((global_end_time - global_start_time).total_seconds() / 600) + 1)
timestamp = datetime.timestamp(global_start_time)


def convert_to_datetime(x):
  return datetime.fromtimestamp(timestamp + x * 600)


date_ticks = [convert_to_datetime(x) for x in num_hours]

fig = ff.create_gantt(df, colors=colors, index_col="Resource", show_colorbar=True, group_tasks=True, width=5000, height=3000,
...
fig.layout.yaxis.update({"tickfont": dict(size=15)})
